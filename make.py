...
def addEglo(date):
    ...
    df = df.dropna()
    df.to_csv("Stany_Kaja" + date +".csv", encoding="utf-8", index=False, header=['Nr_katalogowy', "Ilosc_produktow"], sep=";", mode="a")
...
def addLightPrestige(date):
    ...
    df['Nr_katalogowy'] = df['Nr_katalogowy'].astype(str).str[0:13]
    df = df.dropna()
    df.to_csv("Stany_Kaja" + date +".csv", encoding="utf-8", sep=";", mode="a", index=False, header=False)
...
def addMwGlasberg(date):
    ...
    df['Nr_katalogowy'] = df['Nr_katalogowy'].astype(str).str[0:13]
    df = df.dropna()
    df.to_csv("Stany_Kaja" + date +".csv", encoding="utf-8", index=False, mode="a", sep=";", header=False)
...
def addRabalux(date):
    ...
    df = df.dropna()
    df.to_csv("Stany_Kaja" + date +".csv", encoding="utf-8", sep=";", index=False, mode="a", header=False)
...
def fixCSV(date):
    df = pd.read_csv("Stany_Kaja" + date + ".csv", encoding="utf-8", sep=";")
...
def makeCSV():
    ...
    addEglo(d)
    addCandelux(d)
    # addHoldBox() stany po SKU
    addItalux(d)
    addLampex(d)
    addLedea(d)
    addLightPrestige(d)
    addMwGlasberg(d)
    # addMaytoni() stany po SKU
    x = checkEkoLight()
    addEkoLight(x,d)
    addRabalux(d)
